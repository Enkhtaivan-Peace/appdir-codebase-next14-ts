...
export const TodoService =  {
    ...
    fetchTodos: async (payload:TListReq) => {
        ...
        const res = await fetchApi.get<ITodo[]>('/todos' + queryParams)
        return res
    },

    fetchTodoById: async (id:number) => {
        const res = await fetchApi.get<ITodo>('/todos/' + id)
        return res
    },

    addTodo: async (data:TCreateTodo) => {
        const res = await fetchApi.post<TCreateTodo>('/todos', data )
        return res;
    },

    editTodo: async (id:number, data:TEditTodo) => {
        const res = await fetchApi.put<TEditTodo>('/todos/' + id, data )
        return res;
    },

    deleteTodo: async (id:number) => { 
        console.log('todoId:', id)
        const res = await fetchApi.delete<TDeleteTodo>('/todos', id)
        return res
    },
